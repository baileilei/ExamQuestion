...
- (void)viewDidLoad
{
    ...
    for (ProblemPaperKindObject *cateObj in [ProblemPaperKindObject share].categorysShow) {
        DropItemObjcet *item=[[DropItemObjcet alloc] init];
        item.title=[NSString stringWithFormat:@"%@",cateObj.Name];
        item.index=cateObj.PRKID;
...
//    self.hideMenuView=[[DropSortMenuView alloc] initWithFrame:CGRectMake(0, lable.frame.origin.y+lable.frame.size.height+20, self.view.bounds.size.width, 200)];
    ...
    for (ProblemPaperKindObject *cateObj in [ProblemPaperKindObject share].categoryHide) {
        DropItemObjcet *item=[[DropItemObjcet alloc] init];
        item.title=[NSString stringWithFormat:@"%@",cateObj.Name];
        item.index=cateObj.PRKID;
...
{
    oldShowCategorys=[NSArray arrayWithArray:[ProblemPaperKindObject share].categorysShow];
...
-(void)updateCategory
{
    ...
    for (DropItemObjcet *item in self.sortMenuView.itemArray) {
        NSPredicate *filter=[NSPredicate predicateWithFormat:@"PRKID=%d",item.index];
        [showCategoryTmp addObjectsFromArray:[[ProblemPaperKindObject share].categorys filteredArrayUsingPredicate:filter]];
        ...
        if (!showCategorysChanged) {
            ...
            {
                ProblemPaperKindObject *obj=[oldShowCategorys objectAtIndex:i];
                if (obj.PRKID!=item.index) {
                    showCategorysChanged=YES;
                }
            }
        }
    ...
    [ProblemPaperKindObject share].categorysShow=[NSArray arrayWithArray:showCategoryTmp];
